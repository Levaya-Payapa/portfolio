...
window.addEventListener("load", () => {
    ...
    const showPlayPrompt = (video) => {
        ...
        secondaryText.style.fontSize = "15px";
...
function updateQuickNavPosition() {
    ...
    if (currentIndex === 0) {
        ...
        if (window.innerWidth >= 1200) {
            ...
            quickNav.style.top = "calc((100vh - 412px)/2)";
        }
    }
}
...
const scrollThreshold = 200; // Minimum time (in milliseconds) between scrolls to avoid excessive sensitivity

window.addEventListener("wheel", (event) => {
    const now = Date.now();
    const deltaTime = now - lastScrollTime;

    if (deltaTime < scrollThreshold) {
        return; // Ignore scroll if it's too soon after the last one
    }
...
// Hovering Over Buttons Shows Alt Text (Responsive for >768px)
...
let tooltipTimeout; // Timer for delay
function showTooltip(event) {
    if (window.innerWidth > 768) { // Only show tooltips for wider screens
...
function hideTooltip() {
    clearTimeout(tooltipTimeout); // Clear the delay timer
    tooltip.style.visibility = 'hidden';
    tooltip.style.opacity = '0';
}
function moveTooltip(event) {
    ...
    let xPosition = event.pageX + offset;
    let yPosition = event.pageY - offset;

    // Check if tooltip would overflow on the right
    if (xPosition + tooltipWidth > pageWidth) {
        xPosition = event.pageX - offset - tooltipWidth; // Move to the left of the cursor
    }

    tooltip.style.left = `${xPosition}px`;
    tooltip.style.top = `${yPosition}px`;
}
document.querySelectorAll('.nav-buttons button img').forEach((img) => {
...
// Block Safari Popup; I couldn't get the icons to work right :(
    const isSafari = () => {
        const ua = navigator.userAgent.toLowerCase();
        return /safari/.test(ua) && !/chrome|edge|opr/.test(ua);
    };
        const showSafariWarning = () => {
        console.log("Safari detected. Displaying warning message.");
    
        // Create and style the warning popup container
        ...
        warningContainer.style.background = "hsl(0, 0%, 15%)";
        warningContainer.style.color = "white";
        warningContainer.style.padding = "150%";
        warningContainer.style.zIndex = "1001";
    
        // Create and style the main message
        const mainText = document.createElement("div");
        mainText.textContent = "Welcome to Levi DeWitt's Motion Graphics Portfolio!";
        mainText.style.fontSize = "22px";
        mainText.style.marginBottom = "1em";
        warningContainer.appendChild(mainText);
    
        // Create and style the secondary message
        const secondaryText = document.createElement("div");
        secondaryText.textContent = "Web development is really hard and, unfortunately, my website doesn't quite work on Safari. For a better experience, please switch to Firefox, Chrome, or Edge. I apologize for the inconvenience.";
        secondaryText.style.fontSize = "15px";
        secondaryText.style.opacity = "0.8";
        warningContainer.appendChild(secondaryText);
    
        // Append the warning container to the document
        document.body.appendChild(warningContainer);
        warningContainer.addEventListener("click", () => {
            warningContainer.remove(); // Remove the warning popup
        });
    };
        if (isSafari()) {
        showSafariWarning();
